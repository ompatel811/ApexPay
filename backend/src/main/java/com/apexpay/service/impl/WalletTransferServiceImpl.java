...
public class WalletTransferServiceImpl implements WalletTransferService {
    ...
    public Transaction executeTransfer(UUID senderWalletId, UUID receiverWalletId, BigDecimal amount, String remarks) {
        log.info("Starting wallet transfer transaction: sender={}, receiver={}, amount={}", senderWalletId, receiverWalletId, amount);
        ...
        log.debug("Acquired pessimistic write locks on both wallets. Sender balance before: {}, Receiver balance before: {}",
                lockedSender.getBalance(), lockedReceiver.getBalance());

        // Double-check balance after lock acquisition (re-validation check inside locking window)
        ...
        try {
            ...
            budgetService.checkAndUpdateBudgetsOnTransaction(lockedSender.getUser().getId(), category, amount, currentMonth);
        } catch (Exception e) {
            log.error("Failed to update budget tracker for user {}", lockedSender.getUser().getId(), e);
        }

        log.info("Wallet-to-wallet transfer successfully committed. Ref: {}, Category: {}", transaction.getTransactionReference(), category);
        return transaction;
    }

    private String determineCategory(String remarks) {
        if (remarks == null) return "OTHER";
        String r = remarks.toLowerCase();
        if (r.contains("food") || r.contains("dinner") || r.contains("lunch") || r.contains("restaurant") || r.contains("swiggy") || r.contains("zomato")) {
            return "FOOD";
        }
        if (r.contains("shop") || r.contains("buy") || r.contains("amazon") || r.contains("flipkart") || r.contains("clothing") || r.contains("groceries")) {
            return "SHOPPING";
        }
        if (r.contains("cab") || r.contains("taxi") || r.contains("uber") || r.contains("travel") || r.contains("flight") || r.contains("train") || r.contains("bus")) {
            return "TRAVEL";
        }
        if (r.contains("recharge") || r.contains("mobile") || r.contains("airtel") || r.contains("jio") || r.contains("phone")) {
            return "RECHARGE";
        }
        if (r.contains("electricity") || r.contains("water") || r.contains("gas") || r.contains("utilities")) {
            return "UTILITIES";
        }
        if (r.contains("rent") || r.contains("wifi") || r.contains("internet") || r.contains("broadband") || r.contains("bill")) {
            return "BILLS";
        }
        if (r.contains("medical") || r.contains("doctor") || r.contains("pharmacy") || r.contains("medicine") || r.contains("hospital")) {
            return "MEDICAL";
        }
        if (r.contains("school") || r.contains("college") || r.contains("tuition") || r.contains("education") || r.contains("fees")) {
            return "EDUCATION";
        }
        if (r.contains("stock") || r.contains("mutual fund") || r.contains("investment") || r.contains("crypto") || r.contains("shares")) {
            return "INVESTMENT";
        }
        if (r.contains("movie") || r.contains("netflix") || r.contains("spotify") || r.contains("game") || r.contains("entertainment")) {
